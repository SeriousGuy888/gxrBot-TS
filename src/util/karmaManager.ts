...
export async function processReactionEvent(messageReaction: MessageReaction, user: User, changeMade: "add" | "remove") {
  ...
  if(!Object.values(EmojiId).includes(emojiId as EmojiId))
    return
  if(!messageAuthor)
    return
  if(user.bot || messageAuthor.id === user.id)
    return

  let change = 1
  if(emojiId === EmojiId.downvote)
    change *= -1
  if(changeMade === "remove")
    change *= -1
